#!/usr/bin/env python3
"""
Analyze the chat_history.db database structure and content
"""
import sqlite3
import json
...
def analyze_chat_db():
    ...
    try:
        ...
        # Get indexes
        print("\n=== INDEXES ===")
        cursor.execute("SELECT name, sql FROM sqlite_master WHERE type='index' AND tbl_name='chat_history'")
        ...
        if count > 0:
            ...
            cursor.execute("SELECT * FROM chat_history ORDER BY timestamp DESC LIMIT 3")
            ...
            cursor.execute("SELECT sender_did, COUNT(*) as msg_count FROM chat_history GROUP BY sender_did ORDER BY msg_count DESC")
            sender_stats = cursor.fetchall()
            print(f"\nMessages per sender:")
            for sender, count in sender_stats:
                print(f"  {sender}: {count} messages")
            
            cursor.execute("SELECT group_id, COUNT(*) as msg_count FROM chat_history GROUP BY group_id ORDER BY msg_count DESC")
